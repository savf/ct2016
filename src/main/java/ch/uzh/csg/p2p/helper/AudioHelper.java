...
public class AudioHelper {
	...
	public AudioHelper(Node node, User sender, Friend receiver) {
		this.node = node;
		this.sender = sender;
	...
	public AudioHelper(Node node, User sender) {
		this.node = node;
		this.sender = sender;
		receiverList = new ArrayList<Friend>();
	}
							...
							if (!mute) {
								...
								for (Friend receiver : receiverList) {
									AudioMessage audioMessage = new AudioMessage(
											sender.getUsername(), receiver.getName(),
											receiver.getPeerAddress(), date,
											EncoderUtils.byteBufferToByteArray(byteBufferList));
									MessageRequest request =
											new MessageRequest(audioMessage, RequestType.SEND);
	...
	public static void removeStoredAudioInfoFrom(String sender, String recipient, Date date,
			Node node) {
		FutureRemove futureRemove = node.getPeer().remove(Number160.createHash(recipient))
				.domainKey(Number160.createHash("audio"))
